import type { SearchOptions, SearchResult } from '@/repositories/base.repository';
...
export interface AuthorDataProvider {
	...
	createAuthor(authorData: Omit<Author, 'slug'> & { slug: string }): Promise<Author | null>;
}

export interface CategoryDataProvider {
	findAll(): Promise<Category[]>;
	findBySlug(slug: string): Promise<Category | null>;
	createCategory(categoryData: Omit<Category, 'slug'> & { slug: string }): Promise<Category | null>;
}
...
export interface PageDataProvider {
	...
	createPage(pageData: Omit<Page, 'slug'> & { slug: string }): Promise<Page | null>;
...
export interface MediaDataProvider {
	...
	createMedia(mediaData: Omit<Media, 'slug'> & { slug: string }): Promise<Media | null>;
...
export interface PostDataProvider {
	...
	createPost(postData: Omit<Post, 'slug'> & { slug: string }): Promise<Post | null>;
}
