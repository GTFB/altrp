...
export class SqliteAuthorProvider implements AuthorDataProvider {
	async findAll(): Promise<Author[]> {
		...
		return rows.map(r => ({ slug: r.slug!, name: r.name!, avatar: r.avatar ?? undefined, bio: r.bio ?? undefined, content: r.contentMarkdown ?? undefined }));
	}
	...
	async findBySlug(slug: string): Promise<Author | null> {
		...
		const r = rows[0];
		return r ? { slug: r.slug!, name: r.name!, avatar: r.avatar ?? undefined, bio: r.bio ?? undefined, content: r.contentMarkdown ?? undefined } : null;
	}
