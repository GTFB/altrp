...
export class SqlitePostProvider implements PostDataProvider {
	async findAll(): Promise<Post[]> {
		...
		return Promise.all(rows.map(async r => ({
			...
			content: r.contentMarkdown ? await parseMarkdown(r.contentMarkdown) : undefined,
	...
	async findWithFilters(filters: PostFilters = {}, sort: PostSortOptions = { field: 'date', order: 'desc' }): Promise<Post[]> {
		...
		if (filters.tags?.length) all = all.filter(p => p.tags?.some(t => filters.tags!.includes(t)));
		if (filters.search) {
			const q = filters.search.toLowerCase();
			all = all.filter(p => p.title.toLowerCase().includes(q) || p.description?.toLowerCase().includes(q) || p.excerpt?.toLowerCase().includes(q));
		}
		return all;
	}

	async findWithPagination(filters: PostFilters = {}, sort: PostSortOptions = { field: 'date', order: 'desc' }, pagination: PaginationOptions = { page: 1, limit: 10 }): Promise<PaginatedResult<Post>> {
		...
		return { data: items, pagination: { page: pagination.page, limit: pagination.limit, total, totalPages, hasNext: pagination.page < totalPages, hasPrev: pagination.page > 1 } };
	}

	async findBySlug(slug: string, locale: string = i18nConfig.defaultLocale): Promise<Post | null> {
		const rows = await db.select().from(posts).where(and(eq(posts.slug, slug), eq(posts.locale, locale))).limit(1);
		const r = rows[0];
		if (!r) return null;
		return {
			slug: r.slug!,
	...
	async findAllAuthors(): Promise<string[]> {
		...
		const set = new Set<string>();
		rows.forEach(r => { if (r.author) set.add(r.author); });
		return Array.from(set).sort();
	}
	...
	async findAllTags(): Promise<Array<{ tag: string; count: number }>> {
		...
		rows.forEach(r => { if (r.tagsJson) JSON.parse(r.tagsJson).forEach((t: string) => counts.set(t, (counts.get(t) || 0) + 1)); });
		return Array.from(counts.entries()).map(([tag, count]) => ({ tag, count })).sort((a, b) => b.count - a.count);
	}

	async search(query: string): Promise<{ item: Post; score?: number | undefined; matches?: string[] | undefined; }[]> {
		const all = await this.findAll();
		const q = query.toLowerCase();
		const items = all.filter(p => p.title.toLowerCase().includes(q) || p.description?.toLowerCase().includes(q) || p.excerpt?.toLowerCase().includes(q));
