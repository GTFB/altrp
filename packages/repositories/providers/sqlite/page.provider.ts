...
import { pages } from '../../../../apps/cms/src/db/schema';
...
export class SqlitePageProvider implements PageDataProvider {
	...
	async findWithFilters(filters: PageFilters = {}, sort: PageSortOptions = { field: 'title', order: 'asc' }): Promise<Page[]> {
	...
	async searchPages(query: string): Promise<Page[]> {
		...
		return all.filter(p => p.title.toLowerCase().includes(q) || p.description?.toLowerCase().includes(q) || p.excerpt?.toLowerCase().includes(q));
	}
