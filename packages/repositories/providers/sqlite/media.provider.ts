...
import { media } from '../../../../apps/cms/src/db/schema';
...
export class SqliteMediaProvider implements MediaDataProvider {
	...
	async findWithFilters(filters: MediaFilters = {}, sort: MediaSortOptions = { field: 'title', order: 'asc' }): Promise<Media[]> {
		let all = await this.findAll();
		if (filters.type) all = all.filter(m => m.type === filters.type);
		if (filters.tags?.length) all = all.filter(m => m.tags?.some(t => filters.tags!.includes(t)));
		if (filters.minSize) all = all.filter(m => (m.size ?? 0) >= filters.minSize!);
		if (filters.maxSize) all = all.filter(m => (m.size ?? 0) <= filters.maxSize!);
		if (filters.search) {
			const q = filters.search.toLowerCase();
			all = all.filter(m => m.title.toLowerCase().includes(q) || m.description?.toLowerCase().includes(q) || m.alt?.toLowerCase().includes(q));
		}
		return all;
	}
	...
	async findAllTags(): Promise<string[]> {
		...
		const set = new Set<string>();
		rows.forEach(r => { if (r.tagsJson) JSON.parse(r.tagsJson).forEach((t: string) => set.add(t)); });
		return Array.from(set).sort();
	}

	async findByType(type: NonNullable<Media['type']>): Promise<Media[]> {
		const rows = await db.select().from(media).where(eq(media.type, type)).limit(1000);
		return rows.map(r => ({
			slug: r.slug!, title: r.title!, url: r.url!, type: r.type as any,
			description: r.description ?? undefined, date: r.date ?? undefined,
			alt: r.alt ?? undefined, size: r.size ?? undefined, width: r.width ?? undefined, height: r.height ?? undefined, duration: r.duration ?? undefined, content: r.contentMarkdown ?? undefined,
			tags: r.tagsJson ? JSON.parse(r.tagsJson) : undefined,
		}));
	}
	...
	async searchMedia(query: string): Promise<Media[]> {
		...
		return all.filter(m => m.title.toLowerCase().includes(q) || m.description?.toLowerCase().includes(q) || m.alt?.toLowerCase().includes(q));
	}
	...
	async getMediaStats(): Promise<{ total: number; byType: Record<string, number>; totalSize: number; averageSize: number; }> {
		const items = await this.findAll();
		const stats = { total: items.length, byType: {} as Record<string, number>, totalSize: 0, averageSize: 0 };
		items.forEach(m => { const t = m.type || 'unknown'; stats.byType[t] = (stats.byType[t] || 0) + 1; if (m.size) stats.totalSize += m.size; });
