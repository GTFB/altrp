...
import type { PageDataProvider } from '../types';
...
export class MdxPageProvider implements PageDataProvider {
	...
	private applyFilters(pages: Page[], filters: PageFilters = {}): Page[] {
		return pages.filter(page => {
			if (filters.tags && filters.tags.length > 0) {
				const hasMatchingTag = page.tags?.some(tag => filters.tags!.includes(tag));
			...
			if (filters.search) {
				...
				const matchesTags = page.tags?.some(tag => tag.toLowerCase().includes(searchLower)) || false;
	...
	private applySorting(pages: Page[], sortOptions: PageSortOptions = { field: 'title', order: 'asc' }): Page[] {
