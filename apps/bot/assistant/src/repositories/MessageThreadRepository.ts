...
export class MessageThreadRepository {
  ...
  /**
   * Get assistants by parent_maid where value IS NULL (not yet created topics)
   */
  async getAssistantsByParentMaid(parentMaid: string, type: string): Promise<MessageThreadData[]> {
    try {
      const result = await this.d1Storage.execute(`
        SELECT id, uuid, maid, parent_maid, title, status_name, type, 
               \`order\`, xaid, value, updated_at, created_at, deleted_at, gin, data_in
        FROM message_threads 
        WHERE parent_maid = ? AND type = ? AND value IS NULL AND deleted_at IS NULL
        ORDER BY \`order\`, created_at
      `, [parentMaid, type]);
      ...
      console.error(`Error getting assistants by parent_maid ${parentMaid} and type ${type}:`, error);
      return [];
    }
  }
