...
export const Archieve: CollectionConfig = {
  slug: 'archieve',
  access: {
  ...
  fields: [
    { name: 'uuid', type: 'text', required: true, index: true, unique: true, validate: (val) => {
      if (typeof val !== 'string') return 'uuid must be a string'
      // v4 UUID simple pattern
      const re = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
      return re.test(val) || 'Invalid UUID v4 format'
    } },
    { name: 'aaid', type: 'text', index: true },
    { name: 'number', type: 'text', index: true, validate: (val) => {
    ...
    { name: 'type_name', type: 'text', index: true },
