...
export function logAuthStatus() {
  ...
  console.log('Required vars:', status.missing.length === 0 ? '✅ All set' : `❌ Missing: ${status.missing.join(', ')}`);
  console.log('OAuth providers:', status.hasOAuth ? `✅ Configured: ${status.configured.length}` : '❌ None configured');
